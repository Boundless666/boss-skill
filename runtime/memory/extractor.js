...
function extractFeatureMemories({ feature, events = [], execution = {}, now }) {
  const records = [];
    ...
    if (event.type === 'StageCompleted' && event.data) {
      ...
      if ((stage.retryCount || 0) === 0) {
        records.push(buildRecord({
          ...
          stage: event.data.stage,
          agent: null,
          summary: `Stable completion with roles=${parameters.roles || 'full'} pack=${parameters.pipelinePack || 'default'}`,
          source: { type: 'execution-parameters', window: 'latest' },
          evidence: [{ type: 'event', ref: String(event.id) }],
          tags: [parameters.roles || 'full'],
          confidence: 0.7,
          createdAt: event.timestamp,
          lastSeenAt: event.timestamp,
          expiresAt: null,
