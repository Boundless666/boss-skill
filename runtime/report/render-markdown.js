...
function gateLabel(name) {
  switch (name) {
    case 'gate0': return 'Gate 0 (代码质量)';
    case 'gate1': return 'Gate 1 (测试)';
    case 'gate2': return 'Gate 2 (性能)';
    default: return name;
  }
}

function renderMarkdown(model) {
  const completedStages = model.stages.filter((stage) => stage.status === 'completed' || stage.status === 'skipped').length;
  const lines = [
    '# 流水线执行报告',
    '',
    '## 摘要',
  ...
  lines.push('', '## 质量门禁', '', '| 门禁 | 状态 | 通过 | 检查项数 | 执行时间 |', '|------|------|------|----------|----------|');
